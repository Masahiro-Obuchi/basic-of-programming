...
let rec make_eki_list ekimei_lst =
  ...
  | { kanji = kanji_; kana = kana_; romaji = romaji_; shozoku = shozoku_ }
    :: rest ->
      { namae = kanji_; saitan_kyori = infinity; temae_list = [] }
      :: make_eki_list rest (* make_eki_list rest *)

let result = make_eki_list global_ekimei_list
...
(* 目的 : ekimei_t 型のリストを受け取ったら、その駅名を使って eki_t 型のリストを作る
   関数をmapと名前のない関数を使って書き直す *)
(* make_eki_list : ekimei_t list -> eki_t list *)
let make_eki_list_map ekimei_lst = List.map(fun ekimei -> match ekimei with
  | { kanji = kanji_; kana = kana_; romaji = romaji_; shozoku = shozoku_ } ->
      { namae = kanji_; saitan_kyori = infinity; temae_list = [] }) ekimei_lst


(* 目的 : eki_t 型のリストと起点（漢字の文字列）を受け取ったら、
   起点のみ saitan_kyoriが0、temae_list　は始点の駅名のみからなるリスト
   （eki_t list）を返す関数をmapと名前のない関数を使って書き直す *)
(* shokika : eki_t -> eki_t *)
let shokika_map eki_lst = List.map(fun eki -> match eki with
  | { namae = namae_; saitan_kyori = kyori_; temae_list = temae_ } ->
      { namae = namae_; saitan_kyori = 0.; temae_list = [ namae_ ] }) eki_lst
