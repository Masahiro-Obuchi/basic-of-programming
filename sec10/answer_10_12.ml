...
(* 目的 : 漢字の駅名を２つ（いずれも文字列）と駅間リスト（ekikan_t list）を受け取ったら、
...
let rec get_ekikan_kyori eki1 eki2 lst =
  ...
  | {
    ...
    :: rest ->
      if (kiten_ = eki1 && shuten_ = eki2) || (kiten_ = eki2 && shuten_ = eki1)
      then kyori_
...
(* 目的 : ローマ字の駅名（文字列）と駅名リスト（ekimei_t list 型）を受け取ったら、
   その駅の漢字表記を文字列で返す *)
...
let rec romaji_to_kanji ekimei_romaji lst =
  ...
  | { kanji = kanji_; kana = kana_; romaji = romaji_; shozoku = shozoku_ }
    :: rest ->
      if romaji_ = ekimei_romaji then kanji_
      else romaji_to_kanji ekimei_romaji rest
(* romaji_to_kanji ekimei_romaji rest *)

(* 目的 : ローマ字の駅名を２つ受け取ってきたら、その間の距離を調べ、直接繋がっている場合は
   「A駅からB駅までは xx km です」という文字列を返し、繋がっていない場合は、「A駅とB駅は
   繋がっていません」という文字列を返す *)
(* kyori_wo_hyoji : string -> string -> ekimei_t list -> ekikan_t list -> string *)
let rec kyori_wo_hyoji eki1_romaji eki2_romaji lst1 lst2 =
  let eki1_kanji = romaji_to_kanji eki1_romaji lst1 in
  let eki2_kanji = romaji_to_kanji eki2_romaji lst1 in
  let kyori = get_ekikan_kyori eki1_kanji eki2_kanji lst2 in
  if kyori = infinity then eki1_kanji ^ "駅と" ^ eki2_kanji ^ "駅は繋がっていません"
  else
    eki1_kanji ^ "駅から" ^ eki2_kanji ^ "駅までは " ^ string_of_float kyori ^ " km です"

(* テスト *)
let test0 =
  kyori_wo_hyoji "yoyogiuehara" "yoyogikouen" global_ekimei_list
    global_ekikan_list
  = "代々木上原駅から代々木公園駅までは 1. km です"

let test1 =
  kyori_wo_hyoji "yoyogikouen" "meijijinguumae" global_ekimei_list
    global_ekikan_list
  = "代々木公園駅から明治神宮前駅までは 1.2 km です"

let test2 =
  kyori_wo_hyoji "kudanshita" "iidabashi" global_ekimei_list global_ekikan_list
  = "九段下駅から飯田橋駅までは 0.7 km です"

let test3 =
  kyori_wo_hyoji "takadanobaba" "ochiai" global_ekimei_list global_ekikan_list
  = "高田馬場駅から落合駅までは 1.9 km です"

let test4 =
  kyori_wo_hyoji "ikebukuro" "kanametyou" global_ekimei_list global_ekikan_list
  = "池袋駅から要町駅までは 1.2 km です"

let test5 =
  kyori_wo_hyoji "ichigaya" "nagatacho" global_ekimei_list global_ekikan_list
  = "市ヶ谷駅と永田町駅は繋がっていません"
