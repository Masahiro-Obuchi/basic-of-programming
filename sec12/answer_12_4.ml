...
(* 目的 : あらかじめ昇順に並んでいる駅名のリスト lst と駅名 eki を受け取ったら、
   eki を挿入したリストを返す *)
(* eki_insert : ekimei_t list ekimei_t -> ekimei_t list *)
let rec eki_insert eki_list eki =
  match eki with
  | { kanji = eki_kanji; kana = eki_kana; shozoku = eki_shozoku } -> (
      match eki_list with
      | [] -> eki :: []
      | ({ kanji = list_kanji; kana = list_kana; shozoku = list_shozoku } as
        first)
        :: rest ->
          if eki_kana > list_kana then first :: eki_insert rest eki
          else eki :: first :: rest)
(* eki_insert rest eki*)

(* 目的 : ekimei_t 型のリストを受け取ったら、それをひらがなの順に整列する *)
(* hiragana_sort : ekimei_t list -> ekimei_t list *)
let rec hiragana_sort lst =
  match lst with
  | [] -> []
  | first :: rest ->
      eki_insert (hiragana_sort rest) first (* hiragana_sort rest *)

let test1 = hiragana_sort [] = []
